...
describe('domainUpdatesToDrizzleQuery', () => {
  ...
  it('should return an empty query if no updates are provided', () => {
    const query = domainUpdatesToDrizzleQuery('test', [], updatedBy)
  ...
  it('should return a query if updates are provided', () => {
    ...
    const gid = 'GID-1'
    const query = domainUpdatesToDrizzleQuery(tableName, [{ gid, name: 'John' }], updatedBy, now)
    ...
    expect(sql).toEqual(
      trimSql(`
        ...
        SET 
          ...
          "updatedBy" = CASE "gid" WHEN 'GID-1' THEN '${JSON.stringify(updatedBy)}' ELSE "updatedBy" END
        WHERE "gid" IN ('GID-1')
      `),
    )
  })
  ...
  it('should ignore updates that only have undefined values', () => {
    ...
    const query = domainUpdatesToDrizzleQuery(
      tableName,
      [{ gid: 'GID_JAMIE', thisUpdateShouldBeIgnored: undefined }],
      updatedBy,
      now,
    )
    ...
    expect(rawSql).toEqual(
      trimSql(`
        ...
        SET 
          ...
          END,
          "updatedBy" = CASE "gid"
            WHEN 'GID_JOHN' THEN '${JSON.stringify(updatedBy)}'
            WHEN 'GID_JANE' THEN '${JSON.stringify(updatedBy)}'
            WHEN 'GID_JOE' THEN '${JSON.stringify(updatedBy)}'
            WHEN 'GID_JAKE' THEN '${JSON.stringify(updatedBy)}'
            ELSE "updatedBy"
          END
        WHERE 
          "gid" IN (
